...
|---|-----|------|---------------|--------|
| 0 | [`01-phase-0-cleanup.md`](./01-phase-0-cleanup.md) | Strip cruft, one router, one design system, feature-based structure | 2–3 | ✅ Done |
| 1 | [`02-phase-1-terminal.md`](./02-phase-1-terminal.md) | Terminal v2: agnoster prompt, command registry, autocomplete, history, `gui` switch | 3–4 | ✅ Done · **+ output de-slop** (near-monochrome views, dashed TUI separators); `gui`→`/home` |
| 2 | [`03-phase-2-elegant-gui.md`](./03-phase-2-elegant-gui.md) | Elegant GUI site: one nav/footer/hero, redesigned pages, `mailto:` contact | 3–4 | ✅ **Done** — single-page home + all sections, typed `site.ts` single source, retire-old-pages, git-profile redesigned, a11y (skip-link/landmarks/focus rings/AA contrast), contact copy+toast, returning-visitor banner, `__root` cleanup. **Deviations (user-chosen):** GUI is warm-**dark** editorial (not light); **single-page** (old routes redirect). |
| 3 | [`04-phase-3-blog-seo-perf.md`](./04-phase-3-blog-seo-perf.md) | MDX blog (terminal + GUI), SSG prerender, SEO, Core Web Vitals | 4–5 | ✅ **Done** (on `feat/revamp-phase-3`) — MDX blog both modes from one source, helmet SEO + JSON-LD + sitemap/RSS/robots, route-level code-split (`/home` ~152 KB gz), web-vitals, CF token proxy (octokit dropped). **Deviation (user-approved):** full static-HTML prerender **deferred** — vite-react-ssg's TanStack support is experimental + breaks lazy routes; robust SEO used instead. **Deploy TODO:** set CF `GITHUB_TOKEN`; rotate old `ghp_…` client token. |
| 4 | [`05-phase-4-optional.md`](./05-phase-4-optional.md) | Optional polish: easter eggs, CRT texture, analytics | open | ☐ Backlog (note: GUI dark-mode item is moot — GUI is already dark) |
