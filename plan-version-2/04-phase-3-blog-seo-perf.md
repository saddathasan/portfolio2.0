...
|------|---------------|--------------|--------------|
| 2026-06-01 | **Post-Phase-3 hardening + UX fixes (on `main` via `version-2`).** **Device routing:** `src/shared/lib/mode.ts` — desktop always loads terminal; mobile (<768px) loads GUI unless the visitor saved `preferredMode="terminal"`; `/` route `beforeLoad` redirects accordingly. **GUI navbar blur:** `--surface-nav` token (light/dark, 82% paper) + `backdrop-blur-md` on TopBar/BlogShell/GitProfile so scrolled content no longer bleeds through; mobile hamburger menu added. **Mobile responsiveness:** terminal banner `clamp()` font + `overflow-x-hidden break-words` + flex-wrap prompt rows; "gui mode" button moved to its own dashed-border line; blog listing reflows (`sm:contents`). **GUI typography:** `word-spacing: 0.12em` on Clash Display h1/h2. **Production desktop blank-page fix:** diagnosed an `immutable`-pinned SPA-fallback HTML served as a hashed JS asset (edge-cache poisoning); fixed via `public/_redirects` (`/assets/* → /404.html 404`), `public/404.html`, chunk rename `ui→libs` (fresh path), `vite:preloadError` one-shot reload in `main.tsx`. Verified live recovery. **Deps:** removed dead `@sentry/browser` + `@shadcn/ui`; moved `@tanstack/router-devtools` to devDependencies (pinned to router 1.132.37). **Fonts:** Clash Display + Uncut Sans variable TTFs → woff2 (**283 KB → 105 KB**, ‑63%), woff2-only `@font-face`, TTFs deleted. **OG image:** designed branded 1200×630 `public/og.png` (warm-paper editorial card, real Clash/Uncut faces, `~ $ whoami` nod), wired `SITE.ogImage` + static OG/Twitter meta in `index.html` for non-JS scrapers. | ✅ (tsc, tests, build, lint) | **Still pending (user action):** (1) **revoke** the old exposed `ghp_…BUNUY` PAT at github.com/settings/tokens; (2) set **`GITHUB_TOKEN`** (no `VITE_` prefix; `repo`+`read:org`, SSO-authorized) in Cloudflare → Pages → Settings → Environment variables (Production); (3) run a **production Lighthouse/CWV** pass once the token is live (needs the deploy). Former optional items og.png (3) and woff2 (4) are now ✅ done. |
| 2026-05-31 | **Phase 3 A–F built on `feat/revamp-phase-3`.** A: MDX pipeline (shared guarded `mdx.vite.ts`), Zod `looseObject` frontmatter schema, build-time post index (gray-matter + reading-time), 2 seed posts. B: GUI blog — `/blog`, `/blog/$category`, `/blog/$category/$slug`, editorial MDX component map, Reveal/Article/prev-next, nav. C: terminal blog — posts mounted in the virtual FS, `ls` TUI table, `cat <slug>` renders the SAME compiled MDX via a monospace component map, `.md` optional, `open <slug>`→GUI. D: `<SEO>` (helmet) meta/canonical/OG/Twitter + Person/WebSite/BlogPosting JSON-LD, `seo-assets.vite.ts` emits sitemap/rss/robots. E: route-level lazy (`createLazyFileRoute`) + lazy Terminal → octokit only on git-profile, `/home` ~152 KB gz; web-vitals reporter; dropped unused Space Grotesk. F: `functions/api/github.ts` token proxy, octokit removed (git-profile 32→3 KB gz), no token in `dist`. | ✅ (tsc, 15 tests, build, lint 0-err) | **Decision:** full static-HTML prerender deferred — vite-react-ssg's TanStack support is experimental + breaks lazy routes. Robust SEO (helmet + sitemap/RSS) chosen instead (user-approved). **TODO before deploy:** (1) set `GITHUB_TOKEN` (repo+read:org, SSO) in CF dashboard; (2) **rotate the old `ghp_…` token** in `.env` — it shipped client-side via `VITE_GITHUB_TOKEN` in prior builds; (3) optional: add `/public/og.png` 1200×630 and point `SITE.ogImage` at it; (4) optional: subset Uncut Sans (201 KB ttf) → woff2. |

## Handoff notes
Phase 3 done bar the deferred prerender. Next: open PR `feat/revamp-phase-3` → merge to `version-2`, then `version-2` → `main` and configure Cloudflare Pages (set `GITHUB_TOKEN`). Lazy routes are incompatible with vite-react-ssg, so if true static prerender is wanted later, use a custom Vite-SSR render script (option C from the 3D decision) rather than vite-react-ssg.
